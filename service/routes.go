...
func getSchemas() *client.Schemas {
	...
	// For providers
	for key, value := range providers.Providers {
		providerConfig := schemas.AddType(key, value.GetProviderConfigResource())
		providerConfig.CollectionMethods = []string{}
		providerConfig = value.CustomizeSchema(providerConfig)
	}
